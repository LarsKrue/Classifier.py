...
#           label isn't a dog.
    ...
    for keys, values in results_dic.items():
        ...
        if enthalten == True:
            ...
            dog_identified = 0
        results_dic[keys]=[values[0], values[1],values[2], dog_identified]
    
    # index 4
    for keys, values in results_dic.items():
        class_label = values[1]
        if any(class_label in s for s in dog_name_list):
            dog_class_identified = 1
        else:
            dog_class_identified = 0
        results_dic[keys]=[values[0], values[1],values[2],values[3], dog_class_identified]
